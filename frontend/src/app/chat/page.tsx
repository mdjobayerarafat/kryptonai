...
const MatrixRain = () => {
  ...
  useEffect(() => {
    ...
    let lastTime = 0;
    const fps = 24; // Lower FPS for matrix effect style and performance
...
const NetworkConnections = () => {
  ...
  useEffect(() => {
    const newConnections = Array.from({ length: 15 }, (_, i) => ({
      id: i,
      x1: Math.random() * 100,
      y1: Math.random() * 100,
      x2: Math.random() * 100,
...
export default function ChatPage() {
  ...
  const loadSession = async (id: string) => {
      ...
      try {
          ...
          const uiMessages: Message[] = Array.isArray(res.data) ? res.data.map((m: any) => ({
              role: m.role,
              content: m.content
          })) : [];
  ...
  return (
    <div className="flex h-screen h-[100dvh] bg-black text-gray-100 font-sans overflow-hidden relative">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 cyber-sidebar flex flex-col transition-all duration-300 transform md:relative md:translate-x-0 overflow-hidden ${sidebarOpen ? 'translate-x-0 w-[300px] md:w-80' : '-translate-x-full w-[300px] md:w-0'}`}>
        {/* Sidebar Glow Effect */}
      ...
      <div className="flex-1 flex flex-col relative w-full">
        ...
        <div className="flex-1 overflow-y-auto pt-24 pb-32 md:pt-28 md:pb-40 px-4 md:px-6 scroll-smooth relative z-10">
        ...
        {/* Enhanced Input Area */}
        <div className="absolute bottom-0 w-full cyber-input-area z-30">
